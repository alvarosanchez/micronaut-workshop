== Getting started with Micronaut and GraalVM (20 minutes)
...
=== Using Micronaut Launch to create a new application (3 minutes)
...
icon:hand-o-right[] Let's create a new application with the following command:
  ...
  curl 'https://launch.micronaut.io/create/default/com.example.micronaut.hello?lang=JAVA&build=MAVEN&test=JUNIT&javaVersion=JDK_21&features=graalvm' --output hello.zip

You can now unzip the file and open the resulting folder in your IDE.

=== Running the application with the Micronaut Maven Plugin (3 minutes)
...
=== Creating a Hello World controller (3 minutes)
...
=== Generating a native executable (5 minutes)
