...
<3> The `@PathVariable` annotation marks the `name` parameter as a path variable.
<4> The `@QueryValue` annotation marks the `uppercase` parameter as a query parameter.
+++ </div></details> +++